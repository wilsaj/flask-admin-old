...
class Admin(Module):
    ...
    `app`
        This is the app that this Admin module will be associated
        with.

    `models`
        This should be either a module or an iterable that contains
        the SQLAlchemy models that will be made available through the
        admin interface.

    `db_session`
        This should be a SQLAlchemy session that has been set up and
        bound to an engine. See the documentation on using Flask with
        SQLAlchemy for more information on how to set that up.

    `model_forms`
        This is a dict with model names as keys, mapped to WTForm Form
        objects that should be used as forms for creating and editing
        instances of these models

    `exclude_pks`
        Set this to True if you don't include primary keys in the
        rendered forms. Primary keys are usually generated by your
        database and you don't want them to change.

    `theme`
        The theme that should be used for rendering the admin module.

    `pagination_per_page`
        This is the number of model instances that will be shown in
        the list view if there are more than this number of model
        instances. Default is 25.
