...
def Admin(models, model_forms={}, include_models=[], exclude_models=[], admin_db_session=None, exclude_pk=True):
    global model_dict
    global form_dict
    global db_session
    ...
    if model_dict:
        form_dict = dict([(k, _form_for_model(v, exclude_pk=exclude_pk)) for k,v in model_dict.items()])
...
def generic_model_add(model_name):
    ...
    model = model_dict[model_name]
    form_for_model = form_dict[model_name]
    ...
    if request.method == 'GET':
        ...
        form = form_for_model(request.form)
        if form.validate():
...
def _form_for_model(model_class, exclude_pk=False):
    """
    ...
    exclude=[]
    if exclude_pk:
        exclude=[_get_primary_key_column(model_class).name]
