...
def Admin(models, model_forms={}, include_models=[], exclude_models=[], admin_db_session=None):
    ...
    if model_dict:
        form_dict = dict([(k, _generate_form_for_model(v)) for k,v in model_dict.items()])
...
def _generate_form_for_model(model_class):
    """
    ...
    """
    form = model_form(model_class, converter=AdminConverter())

    # For each field with a foreign key constraint, set up a SelectField widget
    for foreign_key in model_class.__table__.foreign_keys:
        ...
        select_field = QuerySelectFieldAsPK(foreign_model.__name__,
                                            ...
                                            allow_blank=False,
                                            get_label=foreign_model.__repr__)
...
class AdminConverter(ModelConverter):
    """
    Subclass of the wtforms sqlalchemy Model Converter that uses custom widgets
    """
    @converts('Date')
